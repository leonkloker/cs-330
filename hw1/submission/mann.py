...
class MANN(nn.Module):
    ...
    def forward(self, input_images, input_labels):
        ...
        x[:, -1, :, 784:] = 0
        x = x.reshape(B, -1, self.num_classes + 784)

        # Step 3: Pass the concatenated set sequentially to the memory-augmented network
        x, _ = self.layer1(x)
        x, _ = self.layer2(x)

        # Step 4: Return the predictions with [B, K+1, N, N] shape
        return x.reshape(B, -1, self.num_classes, self.num_classes)
    ...
    def loss_function(self, preds, labels):
        ...
        pred_query = preds[:, -1, :, :].reshape(-1, self.num_classes)

        # Step 2: extract the true labels for the query set and reverse the one hot-encoding  
        label_query = labels[:, -1, :, :].reshape(-1, self.num_classes).argmax(dim=1)

        # Step 3: compute the Cross Entropy Loss for the query set only!
        loss = F.cross_entropy(pred_query, label_query)
