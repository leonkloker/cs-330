...
class DataGenerator(IterableDataset):
    ...
    def _sample(self):
        ...
        labels_and_images = get_images(character_folders, np.eye(self.num_classes), num_samples=self.num_samples_per_class)
        ...
        images = np.array([image for label, image in labels_and_images])
        labels = np.array([label for label, image in labels_and_images])

        query_images = [image for i, (label, image) in enumerate(labels_and_images) if i%self.num_samples_per_class==0]
        support_images = [image for i, (label, image) in enumerate(labels_and_images) if i%self.num_samples_per_class!=0]
        query_labels = np.array([label for i, (label, image) in enumerate(labels_and_images) if i%self.num_samples_per_class==0]).reshape(-1, self.num_classes)
        support_labels = np.array([label for i, (label, image) in enumerate(labels_and_images) if i%self.num_samples_per_class!=0]).reshape(self.num_samples_per_class-1,
                                                                                                             self.num_classes,
                                                                                                             self.num_classes)
        query_images = np.array([self.image_file_to_array(image, self.dim_input) for image in query_images]).reshape(-1, 784)
        support_images = np.array([self.image_file_to_array(image, self.dim_input) for image in support_images]).reshape(self.num_samples_per_class-1, 
                                                                                                  self.num_classes,
                                                                                                  784)
        
        # Step 4: Shuffle the order of examples from the query set
        idx = np.random.permutation(self.num_classes)
        query_images, query_labels = query_images[idx][None], query_labels[idx][None]
        images = np.concatenate([support_images, query_images], dtype=np.float32)
        labels = np.concatenate([support_labels, query_labels], dtype=np.float32)
