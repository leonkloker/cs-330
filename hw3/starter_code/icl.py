    ...
    **Note**: Be sure to shuffle the support examples and labels
    ...
    for i in permutation:
        if prompt_mode == "babi":
            ...
            prompt += "Article: " + support_inputs[i] + ". Summary: " + support_labels[i] + ". "
...
def do_sample(
    ...
    with torch.inference_mode():
        ...
        for _ in range(max_tokens-1):
            next_token = model.forward(input_ids).logits[:, -1, :].argmax()

            if int(next_token) in stop_tokens:
                break
                ...
                for k in ks:
                    ...
                    for repeat in range(args.repeats):
                        ...
                        for row in pbar:
                            ...
                            decoded_prediction = tokenizer.decode(do_sample(model, input_ids, stop_tokens, max_tokens))
                            # END YOUR CODE
